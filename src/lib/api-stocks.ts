...
export const getAllStocks = async (userId: string) => {
  ...
  if (Array.isArray(data)) {
    return data.map((stock) => ({ ...stock, finalchange: stock.finalchange ?? 0 }));
  }
  if (Array.isArray(data?.stocks)) {
    return data.stocks.map((stock) => ({ ...stock, finalchange: stock.finalchange ?? 0 }));
  }
...
export const addWatchList = async (userId: string, stockId: number) => {
  const response = await axios.post(`${API_URL}/addWatchList`, {
    user_id: userId,
    stock_id: stockId,
  });
  const data = response.data;

  if (data?.message?.toLowerCase().includes('success') || data?.status === 'success') {
    return data;
  }
...
export const buyStock = async (userId: string, stockId: number, quantity: number, price: number) => {
  ...
  if (data?.message?.toLowerCase().includes('success') || data?.transaction_id || data?.status === 'success') {
    return data;
  }
...
export const sellStock = async (userId: string, stockId: number, quantity: number, price: number) => {
  const response = await axios.post(`${API_URL}/sellStock`, {
    user_id: userId,
    stock_id: stockId,
    quantity,
    price,
